DROP DATABASE IF EXISTS estancias;
CREATE DATABASE IF NOT EXISTS estancias;
USE estancias;
...
select * from casas where fecha_desde like "%2020-08-01%" AND fecha_hasta like "%2020-08-31%";
...
on co.id_casa = e.id_casa;

select  c.nombre, c.ciudad, c.pais, ca.* from clientes c
inner join estancias e
on e.id_cliente= c.id_cliente
inner join casas ca 
on ca.id_casa = e.id_casa;

select precio_habitacion+(precio_habitacion*0.5), id_casa from casas
where pais = "Reino Unido";

select count(id_casa), pais
from casas
group by pais;

select * from casas ca
inner join comentarios co
on ca.id_casa = co.id_casa
where co.comentario like "%limpia%";
