...
FROM PEDIDO pe 
INNER JOIN PAGO p
ON pe.codigo_cliente = p.codigo_cliente
WHERE estado="Rechazado" and fecha_pago LIKE "%2009%";
/*Ejercicio 12*/
SELECT *
FROM PEDIDO pe 
INNER JOIN PAGO p
ON pe.codigo_cliente = p.codigo_cliente
WHERE estado="Entregado" and DATE_FORMAT(fecha_pago,"%c" )="1";
/*Ejercicio 13*/
SELECT * FROM PAGO
...
ON c.codigo_cliente <> p.codigo_cliente;
/*Ejercicio 4*/
SELECT DISTINCT c.nombre_cliente , e.nombre, e.apellido1, e.apellido2, c.ciudad
FROM CLIENTE c 
INNER JOIN EMPLEADO e 
ON c.codigo_empleado_rep_ventas= e.codigo_empleado
INNER JOIN PAGO p
